...
def graceful_kill_vllm(timeout=10):
    ...
    subprocess.run(["pkill", "-15", "vllm"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    
    # Step 2: Wait for processes to exit
    for _ in range(timeout):
        # Check if any vllm process is still running
        result = subprocess.run(["pgrep", "vllm"], capture_output=True)
    ...
    subprocess.run(["pkill", "-9", "vllm"], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
...
def setup_vllm(is_prefill, node_num, base_api_port):
    env = os.environ.copy()
...
def strart_vllm_mock(prefill_num=1, prefill_base_api_port=8000, decode_num=1, decode_base_api_port=9000):
    processes = []
    log_files = []

    proc_list, log_list = setup_vllm(True, prefill_num, prefill_base_api_port)
    processes.extend(proc_list)
    log_files.extend(log_list)
    proc_list, log_list = setup_vllm(False, decode_num, decode_base_api_port)
    processes.extend(proc_list)
    log_files.extend(log_list)
...
if __name__ == "__main__":
    ...
    if len(args) == 0:
        strart_vllm_mock(prefill_num=1, decode_num=1)
    elif len(args) == 4:
        try:
            numeric_args = []
            for arg in args:
            ...
            print("Usage: python script.py <prefill_num> <prefill_base_api_port> <decode_num> <decode_base_api_port>")
            sys.exit(1)
        ...
        print("Usage:")
        print("  python run_vllm_mock.py <prefill_num> <prefill_base_api_port> <decode_num> <decode_base_api_port>  # Start with 4 numbers")
